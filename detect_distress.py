...
# CLASS_NAMES = ['pothole', 'crack', 'patch'] # Example classes
...
# # You can put paths to single images, a list of images, or even a directory.
# # For demonstration, let's use a sample image path.
# # Replace 'path/to/your/new_pavement_image.jpg' with an actual path to an image
# # that the model has NOT seen during training or validation.
# NEW_IMAGE_PATHS = [
#     'path/to/your/new_pavement_image_1.jpg',
#     'path/to/your/new_pavement_image_2.jpg',
#     # Add more image paths as needed
#     # Or, if you want to process all images in a folder:
#     # 'path/to/a/folder_of_new_images'
# ]

# # Set confidence threshold (how sure the model must be to show a detection)
# # Adjust this value based on your desired trade-off between false positives and false negatives.
# CONFIDENCE_THRESHOLD = 0.50 # 0.25 is default, 0.50 is common for higher precision

# # Set IoU threshold for Non-Maximum Suppression (NMS)
# # NMS removes duplicate bounding boxes for the same object.
# IOU_THRESHOLD = 0.45 # Common default
...
#     # Perform inference
#     # 'stream=True' can be more memory efficient for many images, but returns a generator.
#     # For single images or a small list, default is fine.
#     # 'save=True' will save the annotated image to 'runs/detect/predict' folder
#     results = model.predict(
#         source=img_path,
#         imgsz=IMG_SIZE,
#         conf=CONFIDENCE_THRESHOLD,
#         iou=IOU_THRESHOLD,
#         save=True, # Saves the annotated image to disk
#         show=False # Don't show pop-up window if running in script without GUI
#     )

#     # --- Process and Visualize Results for Each Image ---
#     for r in results: # 'results' will be a list of Results objects, usually one per image
...
#         scores = r.boxes.conf.cpu().numpy() # Confidence scores for each box
#         class_ids = r.boxes.cls.cpu().numpy().astype(int) # Predicted class IDs

#         # Create a copy to draw on, so we don't modify the original
...
#             color = (0, 255, 0) # Green for detection boxes (BGR format)
#             label = f"{class_name}: {score:.2f}"

#             # Draw bounding box
#             cv2.rectangle(img_annotated, (x1, y1), (x2, y2), color, 2) # (image, pt1, pt2, color, thickness)

#             # Draw label background (optional, for better readability)
#             (text_width, text_height), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 2)
#             cv2.rectangle(img_annotated, (x1, y1 - text_height - 10), (x1 + text_width, y1), color, -1) # Filled rectangle

#             # Put label text
#             cv2.putText(img_annotated, label, (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2) # Black text
